...
> [!WARNING]
> **The bucket lives on the Mixer, encrypted at rest.** A Coat Check bucket is a key-value bucket of the bus, so the parked context travels to the Mixer over the bus whatever `storage` says. With `storage: "file"` it is written to the Mixer's disk and is encrypted there by default (`snake.store_encryption`), as every message of the bus is; see [data at rest](../../architecture/security.md#data-at-rest-and-in-logs). With `storage: "memory"` it stays in the memory of the Mixer process and never reaches a disk. Either way the Mixer holds the key or the memory, so it reads the context, and so can a client of the bus. Parking regulated data (PAN, PIN blocks, track data) is acceptable when the store encryption is on and the Mixer is inside your cardholder data scope: keep `snake.store_encryption` on, give the Mixer an operator key file for production, and run it hardened (memory locked against swap, core dumps disabled). A bucket held in the memory of the Rack itself is `[Roadmap]`.
