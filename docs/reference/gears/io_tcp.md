...
| `framing` | enum: `delimiter`, `length_prefix2`, `length_prefix4` |  | `delimiter` | message framing mode: delimiter (newline, etc), length_prefix2 (2-byte BE), length_prefix4 (4-byte BE). |
| `idle_timeout` | string |  | `60s` | max time a connection may sit idle before it is closed. |
| `max_buffered_messages` | integer |  | `1000` | server mode: outbound messages queued when nothing is connected yet, or when a message names no conn.id (0 selects the default). |
| `max_connections` | integer |  | `4096` | server mode: maximum concurrent connections (0 = unlimited). |
| `read_responses` | boolean |  | `true` | client mode: whether to read responses from the server. Set to false for one-way send (e.g. to an echo server that doesn't send proper ISO8583 responses). |
